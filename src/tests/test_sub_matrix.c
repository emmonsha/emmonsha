...
START_TEST(sub_equal_size_case) {
...
START_TEST(sub_not_equal_sizes_case) {
  srand((unsigned int)time(NULL));
  int rows = rand() % 100 + 1;
  int usleep(__useconds_t usec);
  usleep(150000);
  ...
  // printf("time = %ld,rows = %d, columns = %d, xrows = %d, xcolumns = %d\n\n",
  // time(NULL), rows, columns, xrows, xcolumns); // НЕ ЗАБУДЬ УДАЛИТЬ ПРОВЕРКУ
...
START_TEST(sub_empty_rows_case) {
  srand((unsigned int)time(NULL));
  int rows = rand() % 100 + 1;
  int usleep(__useconds_t usec);
  usleep(150000);
...
Suite *suite_sub_matrix(void) {
  Suite *s = suite_create("matrix_sub_suite");
  TCase *tc = tcase_create("case_sub_matrix");

  tcase_add_loop_test(tc, sub_equal_size_case, 0, 10);
  tcase_add_loop_test(tc, sub_not_equal_sizes_case, 0, 10);
  tcase_add_loop_test(tc, sub_empty_rows_case, 0, 10);
